# Updated by Kelton Ashe, March 2022
# Updated calculations to account for new sensor position, updated GUI to include gait visualizer tab 
...
import numpy as np
#import beepy
from Knee_Sensor_Data_Stream import kneeSensorStream

from pyquaternion import Quaternion

from kivy.core.audio import SoundLoader

Window.size = (20*40, 20*40) #remove for android builds, will resize to screen automatically
        ...
        ScreenManager:
            ...
            ConnectionScreen:
                ...
                MDTextField:
                    ...
                    helper_text: 'IP Address of Motion-X Device'
                ...
                MDTextField:
                    ...
                    helper_text: 'Port number of Motion-X Device'
                    helper_text_mode: 'on_focus'
                ...
                MDRaisedButton:
                    id: connectButton
                    text: 'Connect to Motion-X Device'
            ...
            BalanceVisualizerScreen:
                name: 'balance_visualizer_screen1'
            ...
            GaitVisualizerScreen:
                name: 'gait_visualizer_screen1'
                ...
                MDBoxLayout:
                    ...
                    MDRectangleFlatButton:
                        ...
                        text: 'Begin Plotting'
                        pos_hint: {'center_x': 0.25, 'center_y': 0.35}
                        on_release: app.gaitPlot()

                    MDRectangleFlatButton:
                        id: stopGraphButton
                        text: 'Stop Plotting'
                        pos_hint: {'center_x': 0.75, 'center_y': 0.35}
                        on_release: app.cancelGaitPlot()

                    Graph:
                        id: graph02 
                        title: 'Gait Visualization'
                        xlabel: 'Time (s)'
                        ylabel: 'Position (m)'
                        xmin: 0
                        xmax: 100
                    ...
                    MDList:
                        OneLineIconListItem:
                            ...
                            IconLeftWidget:
                                icon: 'access-point'
                        OneLineIconListItem:
                            text: 'Balance Visualizer'
                            on_release: screen_mgr1.current = 'balance_visualizer_screen1'
                            IconLeftWidget: 
                                icon: 'axis-arrow'
                        OneLineIconListItem:
                            text: 'Gait Visualizer'
                            on_release: screen_mgr1.current = 'gait_visualizer_screen1'
                            IconLeftWidget: 
                                icon: 'axis-arrow'
                        OneLineIconListItem:
                            text: 'Settings'
...
class BalanceVisualizerScreen(Screen):
    pass

class GaitVisualizerScreen(Screen):
    pass
...
class sweApp(MDApp):
    ...
    def build(self):
        ...
        self.sens_height = 0.49275*self.height/100 #height of device, floor to waist of user 
    ...
    def switchToBalanceVisualizer(self, *args):
        self.root.ids.screen_mgr1.current = 'balance_visualizer_screen1'

    def switchToGaitVisualizer(self, *args):
        self.root.ids.screen_mgr1.current = 'gait_visualizer_screen1'
    ...
    def emergencyAlert(self, *args):
       ...
       #     emerg_email, 'sweNotify', self.username) # FALL GUYS - check username output 
    ...
    def countdownAlert(self, *args):
        for seconds in range(2*self.alertTime + 1):
            Snackbar(
                text=f'{seconds/2} / 5 Elapsed').open()
            time.sleep(0.5)
        self.logData()  # logs the data regardless of false positive/negative status as thread never closes
    ...
    def call_imu_data(self): #used in the button that begins imu connection
        ...
        Snackbar(
            text=f'Attempting connection with {ip_input} : {port_input}... this may take some time').open()

        try:
            self.imu_instance = client_fx(2, ip_input, port_input)
            Snackbar(text=f'Connection with {ip_input} : {port_input} successful!',
                    button_text="Balance Visualizer Screen",
                    button_callback=self.switchToBalanceVisualizer
                    ).show()
        except: 
            Snackbar(text=f'Cannot connect with {ip_input} : {port_input}... try again later').open()
        ...
        with open("c:/Users/kelto/OneDrive/Capstone/Project Code/sweFinalRepo-main/storedDetails/stored_details.txt", 'w') as new_detail:
    ...
    def updatePoints(self, *args):
        ...
        self.c_time = instant_imu[0] - 1641460839 #Equates to time in s since beg of 2021 (roughly)


        
        #IF USING VERTICAL IMU ORIENTATION, NEED TO REDO FRAME CORRECTION TO MATCH SENSOR ORIENTATION
    ...
    def gaitPoints(self, *args):
        if self.plot_init == False: #run this only if graph needs initialized, creates plot objects
            self.kneeX = []
            self.kneeY = []
            self.kneeZ = []
            self.plot_kneeX = SmoothLinePlot(color=[0, 1, 0, 1])
            self.plot_kneeY = SmoothLinePlot(color=[1, 1, 0, 1])
            self.plot_kneeZ = SmoothLinePlot(color=[0, 1, 1, 1])
            self.kneeX, self.kneeY, self.kneeZ = kneeSensorStream()  
            time = range(0, len(self.kneeX)-1)      

            self.plot_kneeX_points = self.kneeX
            self.plot_kneeY_points = self.kneeY
            self.plot_kneeZ_points = self.kneeZ

            
            self.root.ids.graph01.add_plot(self.plot_kneeX)
    ...
    def gaitPlot(self, *args):
        self.root.ids.gait_indicator01.text = 'Gait Stream Active'
        self.graph_activate = True  
        Clock.schedule_interval(self.gaitPoints, 1/self.sensor_refresh)
    ...
    def cancelGaitPlot(self, *args):
        self.root.ids.gait_indicator01.text = 'Gait Stream Standby'
        self.time_session_end = time.asctime()
        #self.logData() - save gait data here?
        Clock.unschedule(self.gaitPoints)  
    ...
    def logData(self, *args):
        ...
        print(filetime)
        with open(r'C:\Users\kelto\OneDrive\Capstone\Project Code\sweFinalRepo-main\dataLog\event_recording_' + filetime +'.csv', 'w', newline='') as f:
